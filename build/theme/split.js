...
const calc = `${['', '-webkit-', '-moz-', '-o-']
  .filter(prefix => {
    const el = document.body.firstElementChild.cloneNode(false); // hack for XSLT transformation
...
const defaultGutterFn = (i, gutterDirection) => {
  var div = document.getElementsByTagName("div")[0]; // hack for xslt
  const gut = div.cloneNode(false);
  while(gut.attributes.length > 0)
    gut.removeAttribute(gut.attributes[0].name);
    ...
    if (!isIE8) {
      ...
      if (i > 0) {
        ...
        if (i == 1 || i === ids.length - 1) {
          const but = gutterElement.cloneNode(false);
          while(but.attributes.length > 0)
            but.removeAttribute(but.attributes[0].name);
