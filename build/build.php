...
CREATE INDEX corpus_document_document ON corpus_document(document);
  ...
  public static function chrono()
  {
    ...
    foreach ($eventList as $event) {
      ...
      if (!$start)
        $start = $event->getAttribute("when");
      $end = $event->getAttribute("to");
      if (!$end)
        $end = $start;
      $lieu_code = $event->getAttribute("where");
      if ($event->parentNode->nodeName != 'event')
        $parent = null;
...
';
      ...
      if ($docs)
        $relations .= "<h3>Documents inclus</h3>\n" . $docs;
      $docs = '';
      ...
      if ($docs)
        $relations .= "<h3>Est inclus dans</h3>\n" . $docs;
      ...
      if ($zevent)
        $relations .= "<h3>Même événement</h3>\n" . $zevent;
      ...
      if ($docs)
        $relations .= "<h3>Même imprimeur</h3>\n" . $docs;


      if ($relations)
        $relations = "<section id=\"doc_rels\">\n<h2>Documents liés</h2>\n" . $relations . "\n</section>";
...
<div class="col-3">
    <nav class="filters">';
    $techniques = self::$pdo->prepare("SELECT id, parent, label, code FROM technique");
    $techniques->execute();
    while ($row = $techniques->fetch(PDO::FETCH_ASSOC)) {
      if (in_array($row['code'], self::$technique_list)) {
        $index .= '<h3 class="filtre">' . $row['label'] . '</h3>' . "\n";
      } else {
        $index .= '<a class="technique" href="#' . $row['code'] . '">' . $row['label'] . '</a>' . "\n";
      }
    }
    $index .= '</nav>';
    $index .= '</div>';
    $index .= '<div class="col-9 techlist">';
    $index .= self::uldocs("technique", null);
    $index .= '</div>
  </div>
  </div>';
...
<div class="object_header">
  <div class="container">
      ...
      if ($row['wikipedia'] || $row['databnf'] || $row['isni']) {
        $page .= '<ul>' . "\n";
        if ($row['wikipedia'])
          $page .= '  <li><a href="' . $row['wikipedia'] . '" target="_new">wikipedia</a></li>' . "\n";
        if ($row['databnf'])
          $page .= '  <li><a href="' . $row['databnf'] . '" target="_new">databnf</a></li>' . "\n";
        if ($row['isni'])
          $page .= '  <li>ISNI : <a href="http://isni.org/isni/' . strtr(
            $row['isni'],
            array(' ' => '', ' ' => '')
          ) . '" target="_new">' . $row['isni'] . '</a></li>' . "\n";
        $page .= '</ul>' . "\n";
      }
      $page .= '
  </div>
...
<p class="notice">
  ...
  public static function corpus()
  {
    ...
    // ===== PHASE 1 : Charger les définitions (sans documents) =====
    ...
    $first = true;
    $ligne_num = 0;
    $corpus_list = array(); // Stocker les corpus pour la phase 2
    ...
    fclose($handle);
    self::$pdo->commit(); // Commiter les définitions

    // ===== PHASE 2 : Ajouter les documents (HORS transaction) =====
    $corpus_doc_insert = self::$pdo->prepare("INSERT INTO corpus_document (corpus_code, document_code) VALUES (?, ?)");

    foreach ($corpus_list as $corpus_def) {
      $code = $corpus_def['code'];
      $titre = $corpus_def['titre'];
      $sql_where = $corpus_def['sql_where'];

      $debug_log .= "<hr><h2>Corpus : $titre</h2>";
      $debug_log .= "<p><strong>Code:</strong> '$code'</p>";
      $debug_log .= "<p><strong>Description:</strong> {$corpus_def['description']}</p>";
      $debug_log .= "<p><strong>SQL WHERE:</strong><br><code>" . htmlspecialchars($sql_where) . "</code></p>";
      ...
      try {
        ...
        // IMPORTANT : Exécuter HORS transaction
        $stmt = self::$pdo->query($sql);
        $count = 0;
        $examples = array();

        // Collecter tous les résultats d'abord
        $documents = array();
        ...
        // Insérer les relations dans une transaction séparée
        if ($count > 0) {
          self::$pdo->beginTransaction();
          foreach ($documents as $doc_code) {
            $corpus_doc_insert->execute(array($code, $doc_code));
          }
          self::$pdo->commit();
        }
        ...
        if ($count > 0) {
          ...
          // Test la sous-requête si elle existe
          if (strpos($sql_where, 'IN (SELECT') !== false) {
            preg_match('/IN \((SELECT[^)]+(?:\([^)]+\))*[^)]*)\)/i', $sql_where, $matches);
            if (isset($matches[1])) {
              $subquery = $matches[1];
              try {
                ...
                if (count($subresults) > 0 && count($subresults) <= 10) {
                  $debug_log .= "<p>Exemples :</p><pre>" . implode("\n", array_slice($subresults, 0, 10)) . "</pre>";
                }
              } catch (PDOException $e) {
                $debug_log .= "<p class='error'>Erreur test sous-requête : " . htmlspecialchars($e->getMessage()) . "</p>";
              }
            }
          }
        }
  ...
  ");
    ...
    // ===== PHASE 4 : Générer les pages HTML =====
  ...
  private static function uldocs($table = null, $id = null)
  {
    ...
    } else if ($table == 'technique' and $id == null) {
      $sql = "SELECT DISTINCT chrono_document.*
      FROM chrono_document
      JOIN technique_document ON technique_document.document = chrono_document.document 
      ORDER BY chrono_document.id;
    ";
      $stmt = self::$pdo->prepare($sql);
    } else if ($table) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
      ...
      if ($table == 'personne')
        $html .= self::htdocument($qdocument->fetch(), $id);
  ...
  private static function htchrono($row)
  {
    ...
    if ($row['start'] != $row['end']) {
      ...
      if (strlen($row['start']) >= 10 && strlen($row['end']) >= 10) {
        ...
        if ($mois1 == $mois2)
          $out .= ', <span clas="day">' . (int) $j1 . '-' . (int) $j2 . '</span> <span clas="month">' . Build::mois($mois1) . '</span>';
        else
          $out .= ', <span clas="day">' . (int) $j1 . '</span> <span clas="month">' . Build::mois($mois1) . ' – ' . (int) $j2 . ' ' . Build::mois($mois2) . '</span>';
      } else {
        if ($mois1 == $mois2)
          $out .= ', <span clas="month">' . Build::mois($mois1) . '</span>';
        else
          $out .= ', <span clas="month">' . Build::mois($mois1) . ' – ' . Build::mois($mois2) . '</span>';
      }
    } else {
      if (strlen($row['start']) >= 10)
        $out .= ', <span clas="day">' . (int) substr($row['start'], 8, 2) . '</span> <span clas="month">' . Build::mois(substr($row['start'], 5, 2)) . '</span>';
      else if (strlen($row['start']) >= 7)
        $out .= ', <span clas="month">' . Build::mois(substr($row['start'], 5, 2)) . '</span>';
    }
    $out .= '.</span>';
  ...
  private static function htdocument($row, $persid = null)
  {
    ...
    if ($persid == "technique") {
      $qtech = self::$pdo->prepare("SELECT DISTINCT technique.code FROM technique_document JOIN technique ON technique_document.technique = technique.id WHERE technique_document.document = ? ORDER BY technique.code");
      $qtech->execute(array($row['id']));
      while ($res = $qtech->fetch(PDO::FETCH_ASSOC)) {
        $techlist[] = $res['code'];
      }
    }
    if ($persid == "lieu") {
...
class Build
{
  ...
  static function pdo($file, $sql)
  {
    ...
    if (!file_exists($file))
      return self::sqlcreate($file, $sql);
    else
      return self::sqlopen($file, $sql);
  }
  ...
  static public function transformDoc($dom, $xslfile, $dst = null, $pars = null)
  {
    ...
    if (!isset(self::$transcache[$key])) {
      ...
      if (defined('XSL_SECPREFS_NONE'))
        $prefs = XSL_SECPREFS_NONE;
      else if (defined('XSL_SECPREF_NONE'))
        $prefs = XSL_SECPREF_NONE;
      else
        $prefs = 0;
      if (method_exists($trans, 'setSecurityPreferences'))
        $oldval = $trans->setSecurityPreferences($prefs);
      else if (method_exists($trans, 'setSecurityPrefs'))
